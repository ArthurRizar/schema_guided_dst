...
from baseline import extract_schema_embedding
...
def _file_based_input_fn_builder(input_dial_file, schema_embedding_file,
    ...
    name_to_features = {
            ...
            "service_id":
                    tf.FixedLenFeature([], tf.int64),
            "utt":
                    tf.FixedLenFeature([max_utt_len], tf.int64),
            "utt_mask":
                    tf.FixedLenFeature([max_utt_len], tf.int64),
            "utt_seg":
                    tf.FixedLenFeature([max_utt_len], tf.int64),
            "cat_slot_num":
                    tf.FixedLenFeature([], tf.int64),
            "cat_slot_status":
                    tf.FixedLenFeature([max_num_cat_slot], tf.int64),
            "cat_slot_value_num":
                    tf.FixedLenFeature([max_num_cat_slot], tf.int64),
            "cat_slot_value":
                    tf.FixedLenFeature([max_num_cat_slot], tf.int64),
            "noncat_slot_num":
                    tf.FixedLenFeature([], tf.int64),
            "noncat_slot_status":
                    tf.FixedLenFeature([max_num_noncat_slot], tf.int64),
            ...
            "req_slot_num":
                    tf.FixedLenFeature([], tf.int64),
            "req_slot_status":
                    tf.FixedLenFeature([max_num_total_slot], tf.int64),
            "intent_num":
                    tf.FixedLenFeature([], tf.int64),
            "intent_status":
...
def _create_dialog_examples(processor, dial_file):
    ...
    if not os.path.exists(FLAGS.dialogues_example_dir):
        os.makedirs(FLAGS.dialogues_example_dir)
...
def _create_schema_embeddings(bert_config, schema_embedding_file):
    ...
    if not os.path.exists(FLAGS.schema_embedding_dir):
        os.makedirs(FLAGS.schema_embedding_dir)
...
def main(_):
    ...
    if not os.path.exists(dial_file) or FLAGS.overwrite_dial_file:
    ...
    if (not os.path.exists(schema_embedding_file) or

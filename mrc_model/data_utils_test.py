...
class Dstc8DataProcessorTest(absltest.TestCase):
    ...
    def test_get_dialog_examples(self):
        ...
        vocab_file = '/root/zhaomeng/google-BERT/uncased_L-12_H-768_A-12/vocab.txt'
        dstc8_data_dir = '/root/zhaomeng/dst_test/dstc8-schema-guided-dialogue' 
        ...
        dialog_paths = [os.path.join(_TEST_DATA_DIR, "train", "dialogues_000.json")]
        dialogs = data_utils.load_dialogues(dialog_paths)
        schema_json_path = os.path.join(_TEST_DATA_DIR, "train", "schema.json")
        schemas = schema.Schema(schema_json_path)

        examples = []
        dataset_type = 'dev'
        for dialog_idx, dialog in enumerate(dialogs):
            cur_examples = processor._create_examples_from_dialog(dialog, schemas, dataset_type)
            examples.extend(cur_examples)
            print(cur_examples[0].intent_status)
            break
